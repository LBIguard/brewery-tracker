    ...
    listItem.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('quick-action-btn')) {
            ...
            if (action === 'visit') {
                ...
                // Update UI
                updateBreweryMarker(breweryObj, markers.find(m => m.brewery.name === breweryName).marker);
                saveToStorage();
...
function handleSwipe(element) {
    ...
    if (swipeDistance < -swipeThreshold) {
        ...
        // Update UI
        updateBreweryMarker(brewery, markers.find(m => m.brewery.name === breweryName).marker);
        saveToStorage();
        ...
        const marker = markers.find(m => m.brewery.name === breweryName).marker;
        ...
        if (navigator.geolocation) {
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                function(error) {
                    ...
                    showToast('Could not get your location. Please check your device settings.', 'error');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('showListBtn').addEventListener('click', function() {
        // Show list, hide map
        document.getElementById('mapContainer').classList.add('h-1/2');
        document.getElementById('mapContainer').classList.remove('h-full');
        document.querySelector('.md\\:w-1\\/3').classList.remove('hidden');
        
        // Update button styles
        this.classList.add('bg-primary', 'text-white');
        this.classList.remove('bg-white', 'dark:bg-gray-700', 'text-gray-700', 'dark:text-white');
        document.getElementById('showMapBtn').classList.remove('bg-primary', 'text-white');
        document.getElementById('showMapBtn').classList.add('bg-white', 'dark:bg-gray-700', 'text-gray-700', 'dark:text-white');
    });
}
